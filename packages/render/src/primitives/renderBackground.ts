...
const drawBackground = (ctx: Context, node: any) => {
  if (!node.box) return;
...
const renderBackground = (ctx: Context, node: any) => {
  ...
  if (hasBackgroundImage) {
    ...
    if (node.no_top) {
      ...
      ctx.fillColor(whiteColor.value as any);
...
const drawBackgroundTemp = (ctx: Context, node: any, color: any) => {
  const xoffset = 0;
  const yoffset = 0;
  ...
  ctx
    .fillOpacity(opacity)
    .fillColor(color.value)
    .rect(left + xoffset, top + yoffset, width, height)
    .fill();
};
